...
| `maintainability` | `complexity` | quick | py, js | any |
...
| **cohesion** | plain text | Per-class cohesion score (0.0–1.0); low scores indicate split-responsibility classes | < 0.5 on a large class → Minor; very low (< 0.3) → Important. **Caveat:** 0.0 on exception classes, ABCs, namespace/config classes, and dataclass-style holders is a metric artifact (no methods sharing instance state), not a real cohesion defect — discount it; only flag low cohesion on classes with substantive behaviour |
...
| **pydeps** | JSON | Dependency map (`stdout` is JSON with module edges); look for unexpected cycles or fan-out | Circular deps → Important; deep fan-out → Minor. **Caveat:** pydeps follows third-party imports and can report package↔package self-cycles (a package importing its own submodules); the raw map is faithful but those are not defects — discount third-party and self-package edges and weigh only cycles among the project's *own* modules |
