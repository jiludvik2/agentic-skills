...
**Active focus:** `epic-analyzer-ga-hardening` (now **8 stories, s0–s7**) → **s0 CLOSED**; **s1-js-toolchain-manifest in progress** (planned 4 tasks; s1-t0 + s1-t1 done, **s1-t2 next**). On branch `ccglass-traffic-analysis`. Remaining findings: F8 eslint (s4), F9 CI masks skipped Node integration tests (s1-t3 + s2/s3/s4), F10 CLI error branches (s5). **s6/s7 (install/uninstall) newly planned 2026-05-30 — awaiting operator plan approval; see Open questions.**
...
- **NEW — s6/s7 install/uninstall plan awaiting approval** (2026-05-30): drafted `s6-install-into-claude` (t0 ADR-0018, t1 wheel-packages the bundle, t2 `polyreview install`) + `s7-uninstall-from-claude` (t0 `polyreview uninstall`); epic children + scope note updated. **Build + test** real commands, **user-level `~/.claude`** target, housed under this epic (operator decisions). Key design seam to confirm in ADR-0018: the wheel does **not** currently ship the skill bundle (only `code_review/` + JSON), and `cache_root()` is CWD-anchored not `~/.claude`-anchored — install places the bundle for discovery, caches stay `setup.sh`/`POLYREVIEW_CACHE_DIR`. **Plan approval is human — execution of s6/s7 has not started.** These are independent of s1–s5; sequence vs. the in-flight s1 is an operator call.
