...
## JS/TS cohesion — declared limitation, not implemented

Python has `cohesion` (LCOM4). There is **no maintained, thin, JSON-emitting JS/TS cohesion
(LCOM) tool** that fits the vendored-npm + raw-capture model. Per G8, JS/TS cohesion is a
**known limitation**, documented in SKILL.md and here — explicitly a scope boundary, not an
oversight. Revisit if a suitable tool emerges (a future ADR/story).
