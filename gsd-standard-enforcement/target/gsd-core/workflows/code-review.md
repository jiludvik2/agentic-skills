...
AUDIT_FLAG=$(echo "$FLAGS_JSON" | node -e "process.stdout.write(String(JSON.parse(require('fs').readFileSync('/dev/stdin','utf-8')).audit))")
```
...
Three-tier scoping with explicit precedence (four tiers when --audit is set):

**Tier 0 — --audit flag (overlay; whole-codebase scope via git ls-files):**

```bash
if [ "$AUDIT_FLAG" = "true" ]; then
  REPO_ROOT=$(git rev-parse --show-toplevel 2>/dev/null)
  REVIEW_FILES=()
  while IFS= read -r file; do
    [ -n "$file" ] && REVIEW_FILES+=("$file")
  done < <(git -C "${REPO_ROOT}" ls-files | sort)
  echo "File scope: ${#REVIEW_FILES[@]} files from --audit (full codebase)"
fi
```

Tiers 1-2 below are guarded with `[ "$AUDIT_FLAG" != "true" ]` so --audit wins.

**Tier 1 — --files override (highest precedence per D-08):**

If FILES_OVERRIDE is set (from --files flag):
```bash
if [ "$AUDIT_FLAG" != "true" ] && [ -n "$FILES_OVERRIDE" ]; then
  REVIEW_FILES=()
  REPO_ROOT=$(git rev-parse --show-toplevel 2>/dev/null)
...
```bash
if [ "$AUDIT_FLAG" != "true" ] && [ -z "$FILES_OVERRIDE" ]; then
