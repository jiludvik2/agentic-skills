...
**Architecture & standards contract (always load):** Before reviewing any source files, check for project-specific standards documentation in `docs/`:

1. **`docs/ARCHITECTURE.md`** — If this file exists, read it and extract all architecture rules, layer constraints, module boundaries, forbidden patterns, and invariants. These define the enforced architecture for this project — applied on every review, not only `--audit` runs.
2. **`docs/STANDARDS.md`** — If this file exists, read it and extract all coding standards, conventions, naming rules, and quality requirements.
3. **`docs/adr/`** — If this directory exists, list all `.md` files and read each one whose status is not `Superseded`, `Rejected`, or `Deprecated`. Each ADR's decision and consequences define what was deliberately chosen — code that contradicts an accepted decision is a finding.

Enforce whatever rules and constraints you find in these documents. Do NOT assume project-specific rules beyond what the documents state — the documents define the standard. Flag violations using standard CR-/WR-/IN- severities and cite the specific source document and section (e.g., "violates ARCHITECTURE.md §Layer Rules — adapters must not aggregate data").

If none of these files or directories exist, skip this step and proceed with generic review only.
</project_context>
...
**5. Load project context:** Read `./CLAUDE.md`, check for `.claude/skills/` or `.agents/skills/`, and load architecture & standards documentation if present (`docs/ARCHITECTURE.md`, `docs/STANDARDS.md`, `docs/adr/` — as described in `<project_context>`).
</step>
